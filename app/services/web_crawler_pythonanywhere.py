...
class PythonAnywhereWebCrawler:
    """
    ...
    def start_crawl(self) -> None:
        ...
        with app.app_context():
            try:
                ...
                print(f"DEBUG: Crawl mode = {self.crawl_job.crawl_mode}")
                logger.info(f"Crawl mode: {self.crawl_job.crawl_mode}")
                ...
                if existing_urls_count == 0:
                    ...
                    # DEBUG: Check if we need to discover MORE URLs from sitemap
                    print(f"DEBUG: Checking for additional URLs in sitemap...")
                    logger.info("Checking sitemap for additional URLs not yet in database")
                    
                    # Force discovery of additional URLs from sitemap
                    additional_discovered = sitemap_service.discover_and_store_urls()
                    print(f"DEBUG: Discovered {additional_discovered} additional URLs from sitemap")
                    logger.info(f"Discovered {additional_discovered} additional URLs from sitemap")
                    
                    # Update count
                    total_urls_count = CrawlUrl.query.filter_by(crawl_job_id=self.crawl_job_id).count()
                    print(f"DEBUG: Total URLs in database now: {total_urls_count}")
                    logger.info(f"Total URLs in database after sitemap check: {total_urls_count}")
                    
                    # DEBUG: Check status of existing URLs
                    url_status_counts = db.session.query(
                        CrawlUrl.is_crawled, 
                        CrawlUrl.is_failed, 
                        db.func.count(CrawlUrl.id)
                    ).filter_by(crawl_job_id=self.crawl_job_id).group_by(
                        CrawlUrl.is_crawled, 
                        CrawlUrl.is_failed
                    ).all()
                    
                    print(f"DEBUG: URL status breakdown:")
                    for is_crawled, is_failed, count in url_status_counts:
                        status = f"crawled={is_crawled}, failed={is_failed}"
                        print(f"DEBUG:   {count} URLs with {status}")
                        logger.info(f"URL status: {count} URLs with {status}")
                
                # Step 2: Handle overwrite mode - reset crawl status
                # FORCE OVERWRITE MODE FOR TESTING - Reset all URLs to uncrawled
                print(f"DEBUG: FORCING overwrite mode to reset crawled URLs")
                logger.info("FORCING overwrite mode for testing - resetting all URLs")
                reset_count = sitemap_service.reset_crawl_status_for_overwrite()
                logger.info(f"Reset crawl status for {reset_count} URLs (forced overwrite)")
                print(f"DEBUG: Reset {reset_count} URLs to uncrawled status")
                
                # Step 3 & 4: Crawl URLs in batches until all discovered URLs are processed
                total_pages_crawled = 0
                
                while total_pages_crawled < self.crawl_job.max_pages and not self.should_stop:
                    # Get next batch of URLs to crawl
                    print(f"DEBUG: Getting batch of URLs for job {self.crawl_job_id}")
                    remaining_limit = self.crawl_job.max_pages - total_pages_crawled
                    
                    try:
                        print(f"DEBUG: Querying database for uncrawled URLs...")
                        urls_to_crawl = CrawlUrl.query.filter_by(
                            crawl_job_id=self.crawl_job_id,
                            is_crawled=False,
                            is_failed=False
                        ).limit(remaining_limit).all()
                        print(f"DEBUG: Database query completed, found {len(urls_to_crawl)} URLs")
                    except Exception as e:
                    ...
                    if not urls_to_crawl:
                        logger.info("No more URLs to crawl - all URLs have been processed")
                        break
                    
                    logger.info(f"Found {len(urls_to_crawl)} URLs to crawl in this batch (total crawled so far: {total_pages_crawled}/{self.crawl_job.max_pages})")
                    
                    # Crawl this batch of URLs
                    for i, crawl_url in enumerate(urls_to_crawl):
                        if self.should_stop or total_pages_crawled >= self.crawl_job.max_pages:
                            break
                        ...
                        if success:
                            ...
                            self.stats['pages_crawled'] += 1
                            print(f"FAST: Crawled {url} ({total_pages_crawled}/{self.crawl_job.max_pages})")
                            
                            # Update progress in database every few pages for real-time tracking
                            if total_pages_crawled % 5 == 0 or total_pages_crawled == 1:
                                try:
                                    self.crawl_job.update_progress(
                                        pages_crawled=total_pages_crawled,
                                        total_discovered_urls=CrawlUrl.query.filter_by(crawl_job_id=self.crawl_job.id).count()
                                    )
                                    db.session.commit()
                                    print(f"PROGRESS: Updated job {self.crawl_job.id} - {total_pages_crawled} pages crawled")
                                except Exception as e:
                ...
                if self.should_stop:
                ...
                elif total_pages_crawled >= self.crawl_job.max_pages:
                    # Check if there are more URLs that we couldn't crawl due to limit
                    ...
                    if remaining_urls > 0:
                        logger.info(f"Crawl completed - reached max pages limit ({self.crawl_job.max_pages}). {remaining_urls} URLs remain undiscovered/uncrawled")
                    else:
                        logger.info(f"Crawl completed - processed {total_pages_crawled} pages (max limit reached)")
                else:
                    self.update_job_status('completed')
                    logger.info(f"Crawl job completed - all discovered URLs processed ({total_pages_crawled} pages crawled)")
...
class CrawlerManager:
    ...
    def __init__(self):
        self.active_crawlers: Dict[int, PythonAnywhereWebCrawler] = {}
        self.crawler_threads: Dict[int, threading.Thread] = {}
    
    def start_crawl_job(self, crawl_job_id: int) -> bool:
        """Start a crawl job using subprocess - allows background processing on PythonAnywhere."""
        import subprocess
        import sys
        import os
        ...
        try:
            ...
            # Update job status to 'running' IMMEDIATELY before starting subprocess
            from app.models.crawl_job import CrawlJob
            crawl_job = CrawlJob.query.get(crawl_job_id)
            if crawl_job:
                crawl_job.status = 'running'
                crawl_job.last_run_at = datetime.now()
                db.session.commit()
                logger.info(f"Updated job {crawl_job_id} status to 'running'")
            
            # Get the path to the standalone crawler script
            current_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(os.path.dirname(current_dir))
            crawler_script = os.path.join(project_root, 'standalone_crawler.py')
            
            # Determine Python executable to use
            # On PythonAnywhere, we need to use the specific Python version
            # DO NOT use sys.executable as it points to uwsgi
            python_exe = 'python3.10'
            
            # Verify python3.10 exists
            try:
                subprocess.run([python_exe, '--version'], check=True, capture_output=True)
                print(f"SUBPROCESS: Using Python executable: {python_exe}")
            except (subprocess.CalledProcessError, FileNotFoundError):
                # Fallback to python3
                python_exe = 'python3'
                print(f"SUBPROCESS: Fallback to Python executable: {python_exe}")
            
            print(f"SUBPROCESS: Starting crawler for job {crawl_job_id} using {python_exe}")
            logger.info(f"Starting crawl job {crawl_job_id} in SUBPROCESS mode")
            
            # Create log files for subprocess output
            log_dir = os.path.join(project_root, 'logs')
            os.makedirs(log_dir, exist_ok=True)
            stdout_log = os.path.join(log_dir, f'crawler_{crawl_job_id}_stdout.log')
            stderr_log = os.path.join(log_dir, f'crawler_{crawl_job_id}_stderr.log')
            
            # Start the crawler as a subprocess with output logging
            with open(stdout_log, 'w') as stdout_file, open(stderr_log, 'w') as stderr_file:
                process = subprocess.Popen(
                    [python_exe, crawler_script, str(crawl_job_id)],
                    cwd=project_root,
                    stdout=stdout_file,
                    stderr=stderr_file,
                    env=os.environ.copy()
            ...
            # Store the process for potential stopping later
            self.active_crawlers[crawl_job_id] = {
                'process': process,
            ...
            print(f"SUBPROCESS: Started crawler process {process.pid} for job {crawl_job_id}")
            print(f"SUBPROCESS: Logs at {stdout_log} and {stderr_log}")
            logger.info(f"Started crawler subprocess {process.pid} for crawl job {crawl_job_id}")
            logger.info(f"Subprocess logs: {stdout_log}, {stderr_log}")
    ...
    def stop_crawl_job(self, crawl_job_id: int) -> bool:
        ...
        try:
            ...
            # Get the subprocess info
            crawler_info = self.active_crawlers[crawl_job_id]
    ...
    def _cleanup_finished_processes(self):
        """Clean up any finished subprocess crawlers."""
        finished_jobs = []
        
        for crawl_job_id, crawler_info in self.active_crawlers.items():
            process = crawler_info['process']
            
            # Check if process has finished
            if process.poll() is not None:
                logger.info(f"Process for crawl job {crawl_job_id} has finished (exit code: {process.returncode})")
                finished_jobs.append(crawl_job_id)
        
        # Remove finished jobs from active crawlers
        for crawl_job_id in finished_jobs:
            del self.active_crawlers[crawl_job_id]
            logger.info(f"Cleaned up finished crawl job {crawl_job_id}")
